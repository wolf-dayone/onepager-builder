...
def katalog_schreiben(daten, module):
    ...
    a("")
    a("**Innerhalb eines Kapitels bleibt die Farbe identisch.** Ein Modul ohne")
    a("eigenes `data-chapter` setzt ein laufendes Kapitel fort und übernimmt")
    a("dessen Farbmodus — es wechselt nie mitten im Kapitel. Nur der Übergang zu")
    a("einem neuen Kapitel darf hell/dunkel umschalten (`tools/pruefen.py`")
    a("meldet einen Bruch als FEHLER unter der Regel `kapitelfarbe`).")
    a("")
